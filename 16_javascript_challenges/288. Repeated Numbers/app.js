...
// if two numbers are repeated the same repeats first
...
function testRepeat(arr){
    ...
    let tempNumbers = arr.reduce((acc, curr, index)=>{
        acc[curr] = acc[curr]?{...acc[curr],amount:acc
            [curr]['amount'] + 1 } : { amount: 1, index };
            let amount = acc[curr].amount;
            let place = acc[curr].index;
            if(amount > max || (amount === max && place <=
                position && amount >1)){
                max = amount;
                value = curr;
                position = place;         
...
// return 5
console.log(testRepeat([6, 5, 5, 10, 10]));
// return 10
